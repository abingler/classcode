...
commandBlock(argNode* args)
{
    ...
    */
    ...
    argNode* tempNode1 = args; //Move these to the top?
    argNode* tempNode2 = args;
    
    printf("You made it this far 1\n");

    while(tempNode1 != NULL){ //Run through args to find some pipes
        if(strcmp(tempNode1->argVal, "|") == 0){ //Did we find a "|"
    ...
    argNode** commandTable = malloc(sizeof(argNode*)*(pipesFound + 1)); //Pipes sepearate commands in table 
    ...
    */
    ...
    while(tempNode1->next != NULL){ //current != NULL //Possibly merge this with previous while loop?
        printf("Looped\n");
        argNode* tempNext = tempNode1->next; //AndrewH won't this mess it up?

        if(strcmp(tempNode1->next->argVal, "|") == 0){ //Next value is string
    ...
    //BE CAREFUL Got a seg fault because i was trying to find argValue of a null
